...
Import-Module $emailSvcUtil -ArgumentList $EmailServiceConnectionString, ${env:EMAILFROM} -Force
...
if ($errorsArr.Count -gt 0) {
    $emailBody = $errorsArr | ConvertTo-Html -Property OSVersion, PSVersion, Module, Name, Exception, RetryException -Head $css -Title "Azure PowerShell Live Test Report" -PreContent "$summarySection<h1>Live Test Error Details</h1>"
}
else {
