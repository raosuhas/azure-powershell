...
function Invoke-LiveTestScenario {
    ...
    try {
        ...
        do {
            try {
                ...
                if ($snrRetryCount -lt $script:ScenarioMaxRetryCount) {
                    ...
                    Write-Error "Failed to execute the live scenario `"$Name`" with error details `"$snrErrorDetails`"." -ErrorAction Continue
        ...
        Write-Error "Error occurred when executing the live scenario `"$Name`" with error details `"$snrErrorDetails`"." -ErrorAction Continue
        $snrCsvData.IsSuccess = $false
